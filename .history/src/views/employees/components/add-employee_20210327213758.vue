  ...
  methods: {
    ...
    async btnOk() {
      ...
      try {
        ...
        // this.$parent  通知父组件 更新数据
        // this.$parent  父组件实例
        this.$parent.getEmployeeList && this.$parent.getEmployeeList() // 之间调用父组件的更新方法
        this.$parent.showDialog = false
        // 这里不用写重置数据  因为 关闭弹层触发了 close事件 close 事件绑定了 btncancel()方法
      } catch (error) {
        console.log(error)
      }
    },
    btnCancel() {
      ...
      // updata:props 名称  这么写的话 可以在父组件 直接用 sync 修饰符处理
    }
  }
}
</script>
