...
import { mapActions } from 'vuex' // 引入vuex 的辅助函数
export default {
  name: 'Login',
  data() {
    ...
    handleLogin() {
      ...
      this.$refs.loginForm.validate(async isOK => {
        if (isOK) {
          try {
            this.loading = true
            // 只有我们通过校验了,我们采取调用action
            await this['user/login'](this.loginForm)
            // 应该登录成功后
            // async 标记的函数  实际上是一个 promise 对象
            // await 下面的代码  都是成功执行的代码
            this.$router.push('/')
          } catch (error) {
            console.log(error)
          } finally {
            // 不论执行的是 try 还是 catch  都去关闭转圈
            this.loading = false
          }
        }
      })
      // ref 可以获取一个元素的DOM 对象
      // ref 作用到组件上的时候,可以获取该组件的实例   也就是 this
    }
  }
}
</script>
