...
export default {
  ...
  methods: {
    ...
    async editRole(id) {
      ...
      this.showDialog = true // 显示弹层 // 为了不出现闪烁的问题 先获取数据 再弹出层
    },
    // 取消
    btnCancel() {
      this.roleForm = {
    ...
    async btnOK() {
      try {
        ...
        // roleForm 这个对象 有ID 就是编辑 没有就是新增
        if (this.roleForm.id) {
          await updateRole(this.roleForm) // 更新数据
        } else {
          // 新增
          await addRole(this.roleForm)
        }
        this.getRoleList() // 从新拉取数据
